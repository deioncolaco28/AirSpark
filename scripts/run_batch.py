...
def run_batch_pipeline(aqi_standard: str = "US_EPA", skip_ml: bool = False):
    start_time = time.perf_counter()
    print("\n=======================================================")
    print("      AirSpark Master Big Data Batch Processing Pipeline ")
    print("=======================================================\n")
    ...
    ParquetStorageManager.write_parquet(
        classified_df,
        output_path="data/processed/integrated_aqi.parquet",
        partition_by=["year", "month"],
        mode="overwrite"
    )
    ParquetStorageManager.write_parquet(daily_analytics_df, "data/processed/daily_analytics.parquet", mode="overwrite")
    ParquetStorageManager.write_parquet(hourly_profile_df, "data/processed/hourly_profile.parquet", mode="overwrite")
    ParquetStorageManager.write_parquet(station_summary_df, "data/processed/station_summary.parquet", mode="overwrite")
    ParquetStorageManager.write_parquet(hotspots_df, "data/processed/hotspots.parquet", mode="overwrite")
    ...
    print("\n-------------------------------------------------------")
    print(f" [SUCCESS] Master Batch Pipeline completed in {format_duration(total_duration)}!")
...
def main():
    ...
    sys.exit(run_batch_pipeline(aqi_standard=args.aqi_standard, skip_ml=args.skip_ml))
